...
This is to certify that this project is my own work, based on my personal efforts in studying and applying the concepts learned.  I have constructed the functions and their respective algorithms and corresponding code by myself. The program was run, tested, and debugged by my own efforts.  I further certify that I have not copied in part or whole or otherwise plagiarized the work of other students and/or persons.
														...
														Pierre Vincent C. Hernandez, DLSU ID# 12043656
****************************************************************************************/
...
// prints characters in a single line repeating format
...
// gets string input for different scenarios (i.e. Add word, Play, etc.)
...
// checks if generated random entry is unique and not yet used
int isUniqueLetter(char gameboardRow[], int nCols, int nRandNum, wordList aEntries)
{
   int col, bUnique = TRUE; // assumes that Entry is unique for the gameboard
	...
	// checks uniqueness only when nCols is the starting index [0]
	if(nCols == 0 && aEntries[nRandNum].use > 0) 
		bUnique = FALSE;
	
   for(col = 0; col < nCols && bUnique; col++) // checks uniqueness when nCol is greater than 0
...
// checks the the number letters left in the aEntries that are not yet used in the gameboard
...
void printGameboard(arrMatrix aGameLetters, int nRow, int nCol, int nCurrentRow)
{
   int row, nSpacing;
   ...
   for(row = 0; row < nRow; row++)
   {
      ...
      printGameboardContent(aGameLetters[row], nCol); // contents
...
// checks if letter input of player exists in the current row of gameboard
...
// displays the contents if the choice box in the maintenance phase
void displayMaintenanceContent(int nNum, char * sString)
{
	printf("%60c%6d.%14s%7c\n", V, nNum, sString, V);	//29 characters
}

// displays all the choices/actions that can be done in the maintenance phase
...
// initializes all the strings to empty strings and integers to zero in aEntries
...
// view borders or acts like a frame for the listed words
...
// displays the clues that can be modified in oneEntry
...
// displays the options on which part of chosen clue that can be modified
...
// gets the new modified phrase for the chosen clue part (relation or relation value)
...
// modification menu of the chosen clue. it modifies the clue
...
// displays the modification choices for the chosen entry
...
// main menu for the modify entry. modifies chosen entry
...
// deletion of the chosen clue to be deleted. updates the number of clues left after
...
// main menu for delete clue action. asks user which word to be deleted a clue with and deletes the chosen clue
void deleteClue(wordList aEntries, int nElem)
{
	...
	if(aEntries[nIndex].numClues == 1 && bValid != -1 && nIndex > -1)
...
// main menu for view clues. asks user which word to view clues
...
// swaps the contents of *sDestination and *sSource
